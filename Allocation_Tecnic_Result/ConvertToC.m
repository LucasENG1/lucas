...
% % replace where adjacent to ) parenthesis
% for lineIndex = 1:length(SymbolicOutput)
%     idxsq = regexp(SymbolicOutput(lineIndex),'\)\^4');
%     if ~isempty(idxsq{1})
%         strIn = SymbolicOutput(lineIndex);
%         idxlp = regexp(strIn,'\(');
%         idxrp = regexp(strIn,'\)');
...
%             startIndex(pwrIndex) = index;
%             %            strPat = strcat(strIn{1}(startIndex(pwrIndex):endIndex(pwrIndex)),'^2');
%             strRep = strcat('pow(',strIn{1}(startIndex(pwrIndex)+1:endIndex(pwrIndex)-1),',4)');
%             %            cellStrPat(pwrIndex) = cellstr(strPat);
%             cellStrRep(pwrIndex) = cellstr(strRep);
%         end
%         for pwrIndex = 1:length(idxsq{1})
...
% for lineIndex = 1:length(SymbolicOutput)
%     strIn = char(SymbolicOutput(lineIndex));
%     [match,idxsq1,idxsq2] = regexp(strIn,'\w*\[\w*\]\^4','match','start','end');
%     [idxsq3] = regexp(strIn,'\[\w*\]\^4','start');
%     if ~isempty(match)
%         for pwrIndex = 1:length(match)
%             strVar   = strIn(idxsq1(pwrIndex):idxsq3(pwrIndex)-1);
%             strIndex = strIn(idxsq3(pwrIndex)+1:idxsq2(pwrIndex)-3);
%             strPat = strcat(strVar,'\[',strIndex,'\]\^4');
%             strRep = strcat('pow(',strVar,'[',strIndex,']',',4)');
%             strIn = regexprep(strIn,strPat,strRep);
%         end
%         SymbolicOutput(lineIndex) = cellstr(strIn);
%     end
...
for lineIndex = 1:length(SymbolicOutput)
    strIn = char(SymbolicOutput(lineIndex));
    [match,idxsq1,idxsq2] = regexp(strIn,'\w*\^4','match','start','end');
    [idxsq3] = regexp(strIn,'\^4','start');
    if ~isempty(match)
        for pwrIndex = 1:length(match)
            strVar   = strIn(idxsq1(pwrIndex)+5*(pwrIndex-1):idxsq2(pwrIndex)-2 +5*(pwrIndex-1));
            strPat = strcat(strVar,'\^4');
            strRep = strcat('pow(',strVar,',4)');
            strIn = regexprep(strIn,strPat,strRep);
        end
        SymbolicOutput(lineIndex) = cellstr(strIn);
    end
