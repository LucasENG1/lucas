function ConvertToC(Name)
%% Define file to read in
fileName = strcat('M_code_',Name,'.txt');
delimiter = '';
...
for lineIndex = 1:length(SymbolicOutput)
    strIn = char(SymbolicOutput(lineIndex));
    [match,idxsq1,idxsq2] = regexp(strIn,'\w*\^3','match','start','end');
    [idxsq3] = regexp(strIn,'\^3','start');
    if ~isempty(match)
        for pwrIndex = 1:length(match)
            strVar   = strIn(idxsq1(pwrIndex)+6*(pwrIndex-1):idxsq2(pwrIndex)-2 +6*(pwrIndex-1));
            strPat = strcat(strVar,'\^3');
            strRep = strcat('powf(',strVar,',3)');
            strIn = regexprep(strIn,strPat,strRep);
        end
        SymbolicOutput(lineIndex) = cellstr(strIn);
    end
...
%% Write to file
fileName = strcat('C_code_',Name,'.txt');
